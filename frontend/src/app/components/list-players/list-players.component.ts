...
export class ListPlayersComponent implements OnInit, OnDestroy {
  ...
  private downloadCsvSubscription: Subscription | undefined;

  constructor(private _playerService: PlayerService, private toastr: ToastrService, private filterService: FilterService) { }

  ngOnInit(): void {
    this.filterSubscription = this.filterService.currentFilter.subscribe(filterData => {
      this.onApplyFilter(filterData);
    });
    this.downloadCsvSubscription = this.filterService.downloadCsv$.subscribe(() => {
      console.log('ListPlayers: evento descarga CSV recibido');
      this.exportToCSV();
    });
    this.getListPlayers();
  }
  ...
  exportToCSV(): void {
    console.log('ListPlayers: exportToCSV llamado');
